...
contract GreenHydrogenToken is ERC20, ERC20Burnable, Pausable, AccessControl, ReentrancyGuard
{
    ...
    event tokens_purchased(address indexed buyer, uint256 token_amount, uint256 cost);
    event tokens_refunded(address indexed user, uint256 token_amount, uint256 refund);
    event price_updated(uint256 new_price);
    event treasury_updated(address new_treasury);

    constructor(address admin, uint256 initial_price_per_token, address initial_treasury) ERC20("GreenHydrogenToken", "GHT")
    {
    ...
    function buy_tokens(uint256 token_amount) public payable whenNotPaused nonReentrant
    {
        require(token_amount > 0, "token_amount > 0");
        uint256 cost = mul_div(token_amount, price_per_token, (10 ** decimals()));
    ...
    // --------------------------
    function refund_tokens(uint256 token_amount) public whenNotPaused nonReentrant
    {
        require(token_amount > 0, "token_amount > 0");

        uint256 refund = mul_div(token_amount, price_per_token, (10 ** decimals()));
    ...
    // --------------------------
    function mul_div(uint256 x, uint256 y, uint256 z) internal pure returns (uint256)
    {
